...
import { generateDateBuckets } from "../utils/generateDateBuckets.js";
...
export default class AnalyticsRepository {
  ...
  async getLeadStageDistribution({
    accountId,
    startDate,
    endDate,
  }: {
  ...
  async getConversionTrend({
    ...
    return data.map((item: any) => {
      ...
      return {
        date: item._id,
        // value: Number(((item.converted / item.total) * 100).toFixed(1)),
  ...
  async getRecentLeads({
    ...
    return await LeadModel.find(query)
      .sort({ createdAt: -1 })
      .limit(limit)
      .select("name source phone stage createdAt")
      .lean();
  }
  ...
  async getLeadStatusAnalytics({
    ...
    return data.map((item) => ({
      ...
      percentage: total ? Number(((item.total / total) * 100).toFixed(1)) : 0,
    }));
  }
  ...
  async getRecentConversations({
    ...
    return await ConversationModel.find(query)
      .sort({ createdAt: -1 })
      .limit(limit)
      .select("platform visitorId lastMessage status createdAt")
      .lean();
  }
  async getPerformanceOverviewTrend({
    accountId,
    module,
    startDate,
    endDate,
    ...
    if (module && module !== "overview") {
      baseQuery["source.name"] = module;
    }

    // LEADS
    const leadsPromise = await LeadModel.aggregate([
      {
        $match: baseQuery,
      },
    ...
    // CONVERSATIONS
    const conversationPromise = await ConversationModel.aggregate([
      {
        $match: {
          ...baseQuery,
          status: "open",
    ...
    // CONVERSION RATE
    const conversionPromise = await LeadModel.aggregate([
      {
        $match: baseQuery,
      },
    ...
    const [leads, conversations, conversions] = await Promise.all([
      leadsPromise,
      conversationPromise,
      conversionPromise,
    ]);

    const dates = generateDateBuckets(startDate, endDate);

    return dates.map((date) => {
      const leadItem = leads.find((i) => i._id === date);
      const conversationItem = conversations.find((i) => i._id === date);
      const conversionItem = conversions.find((i) => i._id === date);

      const conversionRate = conversionItem?.total
        ? Number(
            ((conversionItem.converted / conversionItem.total) * 100).toFixed(
              1,
            ),
          )
        : 0;
      ...
      return {
        ...
        conversations: conversationItem?.count || 0,
        conversionRate,
      };
    });
  }

  async countActiveBots({ accountId }: { accountId: string }) {
    return await ChatbotModel.countDocuments({
      accountId,
      status: true,
    });
  }

  async countBotInteractions({
    accountId,
    startDate,
    endDate,
  }: {
...
//   StatusCount,
//   LeadsOverTimeItem,
//   HourlyEngagementItem,
//   WeeklyEngagementItem,
//   MonthlyEngagementItem,
//   RecentLead,
...
//   async countLeads(
//     accountId: string,
//     opts: { createdAfter?: Date; createdBefore?: Date } = {},
//   ): Promise<number> {
//     const q: any = { accountId: new ObjectId(accountId) };
//     if (opts.createdAfter)
//       q.createdAt = { ...(q.createdAt ?? {}), $gte: opts.createdAfter };
//     if (opts.createdBefore)
//       q.createdAt = { ...(q.createdAt ?? {}), $lte: opts.createdBefore };
...
//     const raw: Array<{ source: string; count: number }> =
//       await LeadModel.aggregate(pipeline).exec();

//     // compute simple trend: compare last 7 days vs previous 7 days (best-effort)
//     const trends = await Promise.all(
//       raw.map(async (r) => {
//         const this7 = await LeadModel.countDocuments({
//           accountId: new ObjectId(accountId),
...
//           },
//         }).exec();
//         const prev7 = await LeadModel.countDocuments({
//           accountId: new ObjectId(accountId),
//           "source.name": r.source,
//           createdAt: {
//             $gte: (() => {
...
//       count: t.count,
//       trend: t.trend,
//       color: colorMap[t.source] ?? "#9CA3AF",
//     }));
//   }
...
//   async hourlyEngagement(
//     accountId: string,
//     opts: { date?: Date } = {},
//   ): Promise<HourlyEngagementItem[]> {
...
//   async weeklyEngagement(
//     accountId: string,
//     opts: { weeks?: number } = {},
//   ): Promise<WeeklyEngagementItem[]> {
...
//     return raw.slice(-weeks).map((r: any, idx: number) => ({
//       week: `Week ${idx + 1}`,
//       leads: r.leads,
...
//   async monthlyEngagement(
//     accountId: string,
//     opts: { months?: number } = {},
//   ): Promise<MonthlyEngagementItem[]> {
...
//     const items = await LeadModel.find({ accountId: new ObjectId(accountId) })
//       .sort({ createdAt: -1 })
//       .limit(limit)
//       .lean()
//       .exec();
...
//         r.leads === 0 ? 0 : Math.round((r.conversions / r.leads) * 1000) / 10;
//       return {
//         channel: c,
//         leads: r.leads,
//         conversions: r.conversions,
...
//   async activeCounts(accountId: string): Promise<{
//     chatbots: number;
//     webforms: number;
//     googleAds: number;
//     social: number;
//   }> {
//     const chatbots = await ChatbotModel.countDocuments({
//       accountId: new ObjectId(accountId),
//       status: true,
//     }).exec();
//     const webforms = await FormModel.countDocuments({
//       accountId: new ObjectId(accountId),
//       status: true,
//     }).exec();
